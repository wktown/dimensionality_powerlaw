...
#only last layer(s)
#need to get dimensions of last layer for various model

resnet18_pt_layers = [f'layer4.{i}.relu' for i in range(2)]

resnet50_pt_layers = [f'layer4.{i}.relu' for i in range(3)]
...
def get_activation_models(pytorch=False, vvs=False, taskonomy=False, pytorch_hub=False, untrained=True, transformers=False):
    if pytorch:
...
def untrained_models():
    ...
    identifier = properties_to_id('MNASNet13', 'None', 'Untrained', 'PyTorch')
    model = wrap_pt(model, identifier)
    yield model, mnasnet_layers
    
    model = regnet_x_400mf(weights=None)
    identifier = properties_to_id('RegNet_400mf', 'None', 'Untrained', 'PyTorch')
    model = wrap_pt(model, identifier)
    yield model, regnet_layers
    
    model = resnext50_32x4d(weights=None)
    identifier = properties_to_id('ResNeXt50', 'None', 'Untrained', 'PyTorch')
    model = wrap_pt(model, identifier)
    yield model, resnet50_pt_layers
...
def transformer_models():
    ...
    identifier = properties_to_id('MaxViT', 'Obj.Class.', 'Untr.', 'Py')
    model = wrap_pt(model, identifier)
    yield model, maxvit_layers
    
    model = maxvit_t(weights="IMAGENET1K_V1")
    identifier = properties_to_id('MaxViT', 'Obj.Class.', 'Sprvsd', 'Py')
    model = wrap_pt(model, identifier)
    yield model, maxvit_layers
